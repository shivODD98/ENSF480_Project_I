...
public class DatabaseInterface {
	...
	/**
	 * Checks the login information of either a registered buyer or a operator. Used by the loginController
	 * @param log is the credentials of the client trying to login
	 * @return the instance of the user the client is or null if the user DNE in the database
	 */
	...
	/**
	 * Deletes the appropriate user from the database using the users user name. Use for Unregistration
	 * @param log is the object that contains the user to be deleted user name
	 * @return the success of the deletion, either true if successful or false is unsuccessful
	 */
	public Boolean deleteUserLg(LoginInfo log) {
		
		String sql = "DELETE FROM users WHERE Username=" + "'"+log.getUsername()+"'" +
				 " AND Password=" + "'"+log.getPassword()+"'";
	...
	public Boolean addRegisterBuyer(User newUser, String pass) {
		...
		if(newUser.getType() == UserType.Operator) {
			...
			sql = "INSERT INTO users(Fname,Lname,Username,Password,Type)" + 
					 "VALUES(" + "'" + newUser.getFirstName() + "'," 
					 + "'" + newUser.getLastName() + "',"
					 + "'" + newUser.getUsername() + "',"
					 + "'" + pass + "',"  
					 + "'" + 2  + "'" + 
	...
	public Boolean addDocument(Document Doc) {
		try {
			...
			sql = "INSERT INTO documents (ISBN, Title, AuthorID, Description, FilePath, Type, Price) " + 
					 "VALUES ('"+Doc.getISBN()+"', '"
					 +Doc.getTitle()+"', '"
					 +authorID.getId()+"', '"
					 +Doc.getDescription() +"','" 
